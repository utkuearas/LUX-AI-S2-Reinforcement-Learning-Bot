...
class TessEnv(gym.Wrapper):
    ...
    def __init__(self, seed = None):
        ...
        #self.action_space = Box(low = 0., high = 1., shape = (2,48,48,46), dtype = np.float32)
    ...
    def __setup_cfg(self, cfg):
        ...
        cfg.ROBOTS["HEAVY"].ACTION_QUEUE_POWER_COST = 0
        cfg.ROBOTS["LIGHT"].ACTION_QUEUE_POWER_COST = 0
        cfg.MIN_FACTORIES = 5
        cfg.MAX_FACTORIES = 5
        cfg.verbose = 0
    ...
    def step(self, actions): # Design Required
        ...
        return new_obs, [reward_0 - reward_1 , reward_1 - reward_0] , done, infos
    ...
    def __calc_lichen_reward(self, timesteps, lichen):
        ...
        return 1 / (timesteps - 1040) ** 2 * lichen
    ...
    def __move_reward(self, location, direction):
        ...
        dig_loc = self.dig_map[:,new_location[0],new_location[1]]
        move_reward = dig_loc[1] + dig_loc[2]
        return move_reward
    
    def __update_consumed_energy(self, direction, agent, unit_type, location):
        ...
        rubble = self.dig_map[0,new_location[0],new_location[1]]
        ...
        power = cons + rubble * per * 100
        if agent == "player_0":
            self.consumed_energy_0 += power / 6000
        elif agent == "player_1":
            self.consumed_energy_1 += power / 6000
        else:
            print("Problem14")
    ...
    def __parse_unit_action(self, action, cargo, location, agent, unit_type):
        ...
        if action[0] == 0:
            ...
            if self.dig_map[0,location[0],location[1]] * 100 <= 20 and self.dig_map[0,location[0],location[1]] * 100 > 0:
                if agent == "player_0":
                    self.gather_reward_0 += 1 / 20
                elif agent == "player_1":
                    self.gather_reward_1 += 1 / 20
    ...
    def __parse_factory_action(self, action, agent):
        ...
        if action[0] == 1:
            ...
            """elif action[1] == 2:
                if agent == "player_0":
                    self.build_reward_0 += .01
                elif agent == "player_1":
                    self.build_reward_1 += .01
                else:
                    print("Problem10")
                return 0
            else:
