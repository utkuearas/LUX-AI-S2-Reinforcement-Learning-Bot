...
class Model(nn.Module):
    ...
    def __init__(self, u_act, f_act, obs_space):
        ...
        self.actor = nn.Sequential(
            layer_init(nn.ConvTranspose2d(32, 32, 3, stride=2, padding=1, output_padding=1)),
            nn.LeakyReLU(),
            layer_init(nn.ConvTranspose2d(32, 32, 3, stride=2, padding=1, output_padding=1)),
            nn.LeakyReLU(),
            layer_init(nn.ConvTranspose2d(32, 35, 3, stride=2, padding=1, output_padding=1), std = .01),
            Transpose((0,2,3,1))
        )
